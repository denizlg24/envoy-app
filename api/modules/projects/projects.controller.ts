...
export async function addMember(c: Context) {
  ...
  if (!projectMember || projectMember.role !== "owner") {
    return c.json({ error: "Unauthorized: Only project owners can add members" }, 403);
  }
...
export async function listMembers(c: Context) {
  ...
  if (!projectMember) {
    return c.json({ error: "Unauthorized: You are not a member of this project" }, 403);
  }
...
export async function removeMember(c: Context) {
  ...
  if (!projectMember || projectMember.role !== "owner") {
    return c.json({ error: "Unauthorized: Only project owners can remove members" }, 403);
  }
...
export async function removeAllMembers(c: Context) {
  ...
  if (!projectMember || projectMember.role !== "owner") {
    return c.json({ error: "Unauthorized: Only project owners can remove all members" }, 403);
  }
